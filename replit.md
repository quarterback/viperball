...
## Recent Changes
- **2026-02-17 (Rushing vs Lateral Yards split)**: Separated player and team yard tracking into `rushing_yards` (from run plays) and `lateral_yards` (from lateral chain plays, credited to final receiver). Added `game_rushing_yards` and `game_lateral_yards` to Player dataclass. Team stats now include `rushing_yards` and `lateral_yards` totals. Box score UI displays both stat lines in markdown and DataFrame views. Player Performance table expanded with Rush Yds, Lat Yds, Lat Thrown, Deflections, Bells, Cov Snaps, KP Tackles, KP Ret Yds columns. Lateral chain now properly tracks touches, laterals thrown, TDs, and yards for all involved players.
