...
- **Starter-First-Look System**: Replaces complex 4-tier role system. Top player at each position group becomes STARTER, rest are ROTATION. Each play rolls a style-dependent probability (78% for ground_pound, 48% for chain_gang) to check starters first. If it hits, only starters compete for the touch. If it misses, everyone competes. Ceiling management prevents monopolization and creates natural backup breakout games. Produces realistic stat sheets: ground_pound FEATURE backs project ~1000-1500 rush yards/season.
