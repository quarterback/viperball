...
def render_export_section(shared):
    ...
    if not standings:
        st.title("Export")
        st.info("No standings data available. Simulate games first.")
        return
    ...
    with tabs[0]:
        ...
        st.download_button(
            ...
            standings_csv,
            file_name=f"{season_name.lower().replace(' ', '_')}_standings.csv",
            mime="text/csv",
        ...
        st.download_button(
            ...
            schedule_csv,
            file_name=f"{season_name.lower().replace(' ', '_')}_schedule.csv",
            mime="text/csv",
        ...
        try:
            sched_with_details = api_client.get_schedule(session_id, completed_only=True, include_full_result=True)
            games_with_details = sched_with_details.get("games", [])
        except api_client.APIError:
            games_with_details = all_games

        context_json = _build_season_context_json(session_id, standings, games_with_details, conferences)
        st.download_button(
            "Download Full Season Context JSON",
            context_json,
            file_name=f"{season_name.lower().replace(' ', '_')}_context.json",
        ...
        with tabs[1]:
            ...
            if team_histories:
                ...
                st.download_button(
                    ...
                    buf.getvalue(),
                    file_name="dynasty_team_histories.csv",
                    mime="text/csv",
            ...
            st.caption("Dynasty save/load through the API is coming soon.")
