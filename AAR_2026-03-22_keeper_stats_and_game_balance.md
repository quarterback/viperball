...
The `_late_down_defensive_clamp` method was returning `round(yards * random.uniform(-0.2, 0.25), 1)` — a float — which accumulated into int-typed stat fields, causing precision drift. Fixed to return `int(round(...))`.

### Play-by-Play Integer Rounding
- Contest resolution now returns `int(round(yards))` instead of `round(yards, 1)`
- Removed micro-jitter (±0.3 noise) that caused fractional yard values
- `play_to_dict()` rounds field_position, yards_to_go, and yards to integers
- Templates round all play-by-play numeric displays

## Known Remaining Issues

- **Game analysis text (post-generation)**: The "Game Analysis" block on box score pages doesn't explain the Delta System (delta yards, delta drives, kill rate, DYE/Power Play/Penalty Kill). The analysis text is generated by a post-processing script (`analyze_game.py`, referenced in changelog) that is no longer present in the repo. When this script is restored or rebuilt, the Delta System section should be added with an explanation like:

  > **DELTA SYSTEM**: In viperball, there are no kickoffs. After scoring, the opposing team gets the ball at the 20-yard line, adjusted by the score differential (leading team starts further back). Delta Yards measures the cumulative field position penalty from this system. Kill Rate = % of delta-penalized drives that result in no score. Power Play = drives while trailing (getting favorable field position). Penalty Kill = drives while leading (starting from adverse field position).
